class ElectionPositionRepository {
  ...
  async addPositionToElection(electionId, positionId, { candidate_count, winners_count, votes_per_voter }) {
  ...
  async removePositionFromElection(electionId, positionId) {
    await this.masterDb.query(
      `DELETE FROM election_positions WHERE election_id = ? AND position_id = ?`,
